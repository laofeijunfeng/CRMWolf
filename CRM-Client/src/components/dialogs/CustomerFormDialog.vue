...
async function refreshConflict(preserveInput: boolean): Promise<void> { if (props.mode !== 'edit' || props.customerId === undefined || loading.value) return; submitError.value = null; lifecycleError.value = null; loading.value = true; try { const latest = await customerApi.getCustomerDetail(props.customerId); loadedVersion.value = latest.version; if (!preserveInput) await applyCustomerDetail(latest); else { lifecycleStatusBaseline.value = latest.status === 0 || latest.status === 1 ? latest.status : null; retainedIndustryInfo.value = latest.industry_info ?? null } } catch (error) { submitError.value = toFeedbackError(error, '客户最新版本'); handleApiError(error, '获取客户最新版本') } finally { loading.value = false } }
