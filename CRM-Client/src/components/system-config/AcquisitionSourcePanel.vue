...
/**
 * AcquisitionSourcePanel.vue - 获客来源配置页面内容
 *
 * 功能：
 * - 展示获客来源列表（ListCard）
 * - 搜索 / 按启用态筛选
...
watch(() => [props.action, props.recordId, sources.value.length] as const, ([action, recordId]) => {
  if (action === 'edit' && recordId !== undefined) {
    const source = sources.value.find(item => item.public_id === recordId)
    if (source !== undefined) handleEdit({ ...source, id: source.public_id })
  }
})
</script>
