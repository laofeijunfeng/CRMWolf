...
import { Search, Plus, Pencil, Trash2, Wand2, ListTree } from 'lucide-vue-next'
...
const filteredMethods = computed(() => {
  ...
  if (filterStatus.value !== 'all') {
    filtered = filtered.filter(method =>
      filterStatus.value === 'true' ? method.is_active === 1 : method.is_active !== 1
    )
  }
...
watch(() => props.active, (active) => {
  if (active) {
    fetchProcurementMethods()
    if (props.action === 'create') showCreateDialog()
  }
})
...
watch(() => [props.action, props.recordId, procurementMethods.value.length] as const, ([action, recordId]) => {
  if (action === 'edit' && recordId !== undefined) {
    const method = procurementMethods.value.find(item => String(item.id) === recordId)
          ...
          >
            ...
            <template #itemActions="{ item }">
              <Button
                ...
                size="icon"
                title="阶段模板"
                @click="router.push(`/settings/procurement-methods/${item.id}/stages`)"
              >
