...
const pickerItems = computed<WorkflowNodePickerItem[]>(() => {
  ...
  return WORKFLOW_NODE_TYPES.map(type => {
...
function graph(): WorkflowGraph {
  return {
    schema_version: 1,
    nodes: nodes.value.map(node => ({ id: node.id, type: node.type, position: node.position, config: node.data.config })),
...
function applyIssues(nextIssues: WorkflowValidationIssue[]): void {
  ...
  for (const issue of nextIssues) if (issue.nodeId !== undefined && !byNode.has(issue.nodeId)) byNode.set(issue.nodeId, issue.message)
  nodes.value = nodes.value.map(node => ({ ...node, data: nodeData(node.data.config, byNode.has(node.id), byNode.get(node.id) ?? '') }))
}
function edgeWithInsertContext(edge: WorkflowEditorEdge): WorkflowFlowEdge {
  return { ...edge, type: 'workflow-insert', data: { insertionContext: getEdgeInsertContext(edge) } }
}
function addNode(type: WorkflowNodeType, position = { x: 120, y: 120 }, context: WorkflowInsertContext = { kind: 'root' }): void {
  const definition = WORKFLOW_NODE_REGISTRY[type]
  ...
  edges.value = result.edges.map(edgeWithInsertContext)
...
function addNodeWithAutomaticContext(type: WorkflowNodeType, position: { x: number; y: number }): void {
  ...
  const context: WorkflowInsertContext = soleTerminal !== undefined && !WORKFLOW_NODE_REGISTRY[type].isTrigger
    ? { kind: 'after-node', sourceNodeId: soleTerminal.id } : { kind: 'root' }
...
function updateSelectedConfig(patch: Record<string, unknown>): void {
  ...
  nodes.value = nodes.value.map(node => node.id === selectedNodeId.value
    ? { ...node, data: nodeData({ ...node.data.config, ...patch }, node.data.hasError, node.data.errorMessage) } : node)
...
function onConnect(connection: Connection): void {
  ...
  const edge: WorkflowEditorEdge = { id: freshId('edge'), source: connection.source, target: connection.target }
  edges.value = [...edges.value, edgeWithInsertContext(edge)]
  applyIssues(validateWorkflow(graph(), workflowName.value))
}
function onDrop(event: DragEvent): void {
  event.preventDefault()
  const type = event.dataTransfer?.getData('application/workflow-node') ?? ''
  if (isKnownType(type)) addNodeWithAutomaticContext(type, screenToFlowCoordinate({ x: event.clientX, y: event.clientY }))
}
function onDragOver(event: DragEvent): void { event.preventDefault() }
function openPicker(context: WorkflowInsertContext = { kind: 'root' }): void {
  insertContext.value = context
  pickerOpen.value = true
}
function openPickerForEdge(edgeId: string): void {
  const edge = edges.value.find(candidate => candidate.id === edgeId)
  if (edge !== undefined) openPicker(edge.data.insertionContext)
}
function selectPickerType(type: WorkflowNodeType): void {
  const position = insertContext.value.kind === 'after-node'
    ? (nodes.value.find(node => node.id === insertContext.value.sourceNodeId)?.position ?? { x: 120, y: 120 })
    : { x: 120, y: 120 }
  addNode(type, { x: position.x + 180, y: position.y }, insertContext.value)
}
function hydrate(detail: WorkflowDetail): void {
  ...
  edges.value = detail.dsl.edges.map(edgeWithInsertContext)
  applyIssues([])
}
async function load(): Promise<void> {
  if (props.workflowId === null) return
  loading.value = true
  try { hydrate(await workflowApi.get(props.workflowId)) } catch { errorMessage.value = '无法加载工作流，请稍后重试。' } finally { loading.value = false }
}
...
function validationIssuesOf(error: unknown): WorkflowValidationIssue[] {
  ...
  return errors.filter((message): message is string => typeof message === 'string').map(message => {
...
async function save(): Promise<void> {
  ...
  try {
    const result = props.workflowId === null
      ...
      : await workflowApi.update(props.workflowId, { name: workflowName.value.trim(), description: description.value.trim() || null, dsl, expected_last_modified_time: loadedLastModified.value ?? '' })
    emit('saved', result)
  } catch (error: unknown) {
    ...
    if (status === 422) {
      ...
      errorMessage.value = backendIssues.length > 0 ? `工作流校验失败：${backendIssues.length} 个问题，请检查配置。` : '工作流校验失败，请检查配置。'
    } else errorMessage.value = status === 409 ? '工作流已被修改，请刷新后重试' : '保存工作流失败，请稍后重试。'
  ...
  <div class="workflow-editor flex h-full min-h-[560px] flex-col" data-testid="workflow-editor">
    <header class="flex items-center gap-3 border-b p-3">
      ...
      <div class="ml-auto flex gap-2">
        <Button type="button" variant="outline" data-testid="workflow-add-node" @click="openPicker()">添加节点</Button>
    ...
    <div class="grid min-h-0 flex-1 grid-cols-[220px_1fr_320px]">
      ...
      <div class="min-h-0" @drop="onDrop" @dragover="onDragOver">
        ...
        >
          <Background />
          <Controls />
          <template #edge-workflow-insert="edgeProps">
            <WorkflowInsertEdge v-bind="edgeProps" @insert="openPickerForEdge" />
          </template>
        </VueFlow>
        <div class="flex flex-wrap gap-2 p-3" data-testid="workflow-terminal-inserts">
          <Button
            v-for="context in terminalContexts"
            :key="context.sourceNodeId"
            type="button"
            variant="outline"
            size="sm"
            :data-testid="`workflow-insert-terminal-${context.sourceNodeId}`"
            @click="openPicker(context)"
          >
    ...
    <WorkflowNodePickerDialog v-model:open="pickerOpen" :node-types="pickerItems" :trigger-used="triggerUsed" @select="selectPickerType" />
  </div>
</template>
