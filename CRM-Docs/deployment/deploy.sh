...
CERT_ZIP="$PROJECT_DIR/crm.apipark.cn.zip"
CERT_DOMAIN="crm.apipark.cn"
...
upload_to_server() {
    ...
    if [ -f "$CERT_ZIP" ]; then
        log_info "上传 SSL 证书..."
        ssh -i "$SSH_KEY" "$SERVER" "mkdir -p $DEPLOY_DIR/certs"
        unzip -p "$CERT_ZIP" "$CERT_DOMAIN/fullchain.cer" | ssh -i "$SSH_KEY" "$SERVER" "cat > $DEPLOY_DIR/certs/fullchain.cer"
        unzip -p "$CERT_ZIP" "$CERT_DOMAIN/$CERT_DOMAIN.key" | ssh -i "$SSH_KEY" "$SERVER" "cat > $DEPLOY_DIR/certs/$CERT_DOMAIN.key"
        ssh -i "$SSH_KEY" "$SERVER" "chmod 644 $DEPLOY_DIR/certs/fullchain.cer && chmod 600 $DEPLOY_DIR/certs/$CERT_DOMAIN.key"
    elif ! ssh -i "$SSH_KEY" "$SERVER" "test -f $DEPLOY_DIR/certs/fullchain.cer && test -f $DEPLOY_DIR/certs/$CERT_DOMAIN.key"; then
        log_error "未找到本地证书包 $CERT_ZIP，且服务器上没有可用证书。"
        exit 1
    else
        log_warning "未找到本地证书包，沿用服务器现有 SSL 证书。"
    fi
