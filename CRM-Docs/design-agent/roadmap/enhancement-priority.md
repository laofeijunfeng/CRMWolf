...
## 本轮 Agent 语义优化落地（2026-09-03）

- Root 统一通过 structured output 判断 QUERY/WORKFLOW/CLARIFY、任务承接和历史待办关系。
- 商机、商机阶段、跟进任务和历史待办的模糊对象选择改为“授权候选集 + LLM ordinal 排序 + confidence/margin 裁决”。
- LLM 选择后统一再次走服务端 resolver；对象消失、权限变化或状态变化时停止写入。
- 删除待办候选的客户名/标题 substring 自动评分；保留 exact ID、格式、权限和状态等确定性校验。
- 正则仅保留结构化 ID、结果序号等语法边界，不参与业务意图分类。
