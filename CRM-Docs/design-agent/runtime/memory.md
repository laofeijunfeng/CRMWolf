...
会话记忆用于承接已确认的客户和当前工作主题，避免每轮重复搜索；它不是 CRM 权威事实，也不能绕过 resolver。记忆中的客户身份通过客户 ID 重新校验，用户明确切换客户时由 Root 开启新任务并替换旧绑定。

候选资源的语义排序只基于本轮用户输入、当前任务上下文和服务端刚加载的授权候选。排序完成后必须再次按真实业务 ID读取并复核，防止使用过期候选。模型排序失败时不清空上下文、不误绑定对象，而是走原有选择或澄清路径。
