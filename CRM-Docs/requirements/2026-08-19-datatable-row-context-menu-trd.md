# 列表行操作入口收敛 TRD（已按 UX 复盘修订）

本文原方案曾把桌面 DataTable 的行操作全部收成右键菜单。结合 CRMWolf 实际业务和 UX 复盘，现修订为：桌面保留紧凑的主操作与“更多”入口，右键仅作为快捷入口。本文承接 [列表行操作改为右键菜单 PRD v1.1](https://apifox666.feishu.cn/wiki/ThaEwhhKPikJaqkecwIcLnZTnEb)，只写实现接法，不重写产品规则。
...
| 九页改造清单与固定列处理、桌面主操作与“更多”接入 | 手机长按、批量操作 |
| 右键 / 键盘 / 点行互斥细节 | 后端权限码、确认框文案改写 |
| 设计规范同步点与测试矩阵 | 具体补丁代码 |

按 UX 复盘后的当前决策：桌面不恢复完整多按钮操作列；当前状态下最多 2 个主操作在行尾轻量显示，其余操作进入可见的“更多”菜单；右键和键盘菜单键 / Shift+F10 打开同一份完整操作菜单；查看详情由行点击承担，不重复显示“查看”；高风险操作必须有明确入口并复用统一确认流程；手机保持 `#mobile-actions`，不做长按；`fixedRightCount` 默认改为 0；空动作不弹菜单；链接和输入框的原生菜单优先；关菜单不得误开详情。

## 1. 范围与约束

| 类型 | 说明 |
| --- | --- |
| 范围内 | DataTable 桌面行菜单、紧凑主操作与“更多”、取消右侧自动固定、九个列表页改接入、设计规范与相关单测 |
...
发布前检查：桌面九页不出现完整多按钮操作列；有可执行动作的行显示最多两个主操作和可见“更多”；右键和 Shift+F10 能打开完整菜单；点行仍进详情；关菜单不进详情；横滑右侧无业务数据列固定；手机卡片操作未变。
...
| 项 | 原因 |
| --- | --- |
| 悬停才出现、无键盘可达性的操作按钮 | 避免把可发现性建立在 hover 上；当前“更多”始终可见 |
...
实施记录：已完成分组函数和菜单内容组件、DataTable 受控菜单与固定列默认值、九页动作接入、桌面主操作与“更多”入口、设计规范与相关单测。后续按 PRD A-01…A-12 手工验证，并重点验证高风险操作的显式入口与统一确认。
