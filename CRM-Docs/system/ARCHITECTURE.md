...
│   ├── customers.py        # 客户路由
│   ├── leads.py            # 线索路由
│   ├── customer_ai.py      # AI 客户助手路由
│   ├── lead_ai.py          # AI 线索助手路由
│   ├── web_assistant.py    # AI Web 助手路由
│   ├── workflow_undo.py    # Workflow 撤销路由
│   └── ...                 # 其他路由
│
├── core/                   # 核心配置
│   ├── config.py           # 配置管理（含 Agent 配置项）
│   ├── database.py         # 数据库连接
│   ├── deps.py             # 依赖注入
│   ├── exceptions.py       # 异常定义
│   ├── redis.py            # Redis 连接
...
│   ├── customer.py         # 客户模型
│   ├── lead.py             # 线索模型
│   ├── operation_log.py    # 操作日志模型（含撤销字段）
│   └── ...                 # 其他模型
│
├── schemas/                # Pydantic Schema
│   ├── customer.py         # 客户 Schema
│   ├── lead.py             # 线索 Schema
│   ├── web_assistant.py    # AI Web 助手 Schema
│   └── ...                 # 其他 Schema
│
├── services/               # 业务逻辑层
...
│   │
│   ├── skills/             # AI 技能处理器
│   │   └── handlers/       # 各类操作 Handler
│   │       ├── base_handler.py
│   │       ├── create_handler.py
│   │       ├── follow_up_handler.py
│   │       ├── status_change_handler.py
│   │       ├── ask_user_handler.py
│   │       ├── get_context_handler.py
│   │       └── handler_factory.py
│   │
│   └── workflow/           # Workflow 编排层
│       ├── workflow_orchestrator.py  # 流程编排器
│       ├── workflow_definitions.py   # 业务流程定义
│       ├── state_machine.py          # 状态机校验
│       ├── business_invariants.py    # 业务不变式
│       ├── session_store.py          # Redis Session 存储
│       ├── guardrails.py             # 置信度护栏
│       ├── trace_context.py          # 全链路追踪
│       ├── undo_service.py           # 撤销服务
│       ├── undo_handlers.py          # 撤销处理器
│       ├── entity_renderer.py        # 实体展示渲染
│       └── agent_executor_pool.py    # 资源隔离 + 速率限制
│
├── constants/              # 常量定义
│   ├── tools.py            # AI 工具定义 + Handler 映射
│   ├── handler_configs.py  # Handler 配置
...
| **services/ai_tool_service.py** | ReAct 循环 + AI 工具调度 | 直接 HTTP 响应（通过 api） |
| **services/skills/handlers/** | 单个操作执行 | HTTP 处理、跨 Handler 调用 |
| **services/workflow/** | 流程编排 + 状态机 + 护栏 | HTTP 处理、直接操作数据库 |
| **models/** | 表结构定义 | 业务逻辑 |
...
```
api/  ──调用──▶  ai_tool_service.py  ──调度──▶  handlers/
  │                     │
  │                     │
  └──调用──▶  workflow/  ──校验──▶  state_machine/business_invariants
                            │
                            └──调用──▶  handlers/  ──调用──▶  crud/
```
...
- api → schemas（校验请求/响应）
- api → ai_tool_service → handlers → crud
- workflow → handlers → crud
- workflow → state_machine / business_invariants（校验）

**禁止调用**：
- crud → api（禁止反向依赖）
- models → services（禁止反向依赖）
- api → models（禁止跨层，必须通过 crud）
- handlers → workflow（禁止反向依赖）
- handlers → api（禁止反向依赖）
...
**版本：1.1 | 最后更新：2026-06-12 | 修改需人工审批**