...
class Lead(Base):
    ...
    contact_phone = Column(String(20), nullable=False, unique=True, comment="联系人手机")
    company_scale = Column(Enum(CompanyScale), nullable=True, comment="团队规模")
    
    owner_id = Column(String(100), nullable=True, comment="负责人ID（飞书用户ID）")
    ...
    creator_id = Column(String(100), nullable=False, comment="创建人ID（飞书用户ID）")
...
class LeadFollowUp(Base):
    ...
    creator_id = Column(String(100), nullable=False, comment="创建人（飞书用户ID）")
    created_time = Column(DateTime, nullable=False, default=func.now(), comment="创建时间")
