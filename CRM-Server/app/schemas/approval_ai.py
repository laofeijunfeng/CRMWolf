...
class ApprovalAIParsedFlow(BaseModel):
    ...
    # B3：单据类型 CONTRACT/PAYMENT/INVOICE/LICENSE/OPPORTUNITY，默认 CONTRACT（兼容既有合同解析行为）
...
class ApprovalAICreateRequest(BaseModel):
    ...
    business_type: str = Field(
        BusinessType.CONTRACT,
        description="流程适用单据类型：CONTRACT/PAYMENT/INVOICE/LICENSE/OPPORTUNITY，默认 CONTRACT",
    )
