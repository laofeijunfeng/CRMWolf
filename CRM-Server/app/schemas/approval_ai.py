...
class ApprovalAIParsedFlow(BaseModel):
    ...
    # B3：单据类型 CONTRACT/PAYMENT/INVOICE/INVOICE_REISSUE/LICENSE/OPPORTUNITY，默认 CONTRACT（兼容既有合同解析行为）
    business_type: str = Field(
        BusinessType.CONTRACT,
        description="流程适用单据类型：CONTRACT（合同，默认）/PAYMENT（回款）/INVOICE（发票）/LICENSE（License申请）/OPPORTUNITY（商机）",
    )
...
class ApprovalAICreateRequest(BaseModel):
    ...
    business_type: str = Field(
        BusinessType.CONTRACT,
        description="流程适用单据类型：CONTRACT/PAYMENT/INVOICE/INVOICE_REISSUE/LICENSE/OPPORTUNITY，默认 CONTRACT",
    )
