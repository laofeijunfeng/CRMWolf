...
class ApprovalListItemResponse(BaseModel):
    ...
    business_type: str = Field(..., description="业务单据类型：CONTRACT / PAYMENT / INVOICE / INVOICE_REISSUE / LICENSE / OPPORTUNITY")
    business_id: int = Field(..., description="业务单据ID")
    business_public_id: Optional[str] = Field(None, description="业务单据对外ID；商机审批返回商机对外ID")
    original_invoice_application_id: Optional[int] = Field(None, description="原发票申请ID；仅发票重开审批返回")
    ...
    has_attachment: Optional[bool] = Field(False, description="是否存在可预览/下载附件")
    contract_file_path: Optional[str] = Field(None, description="合同附件路径，仅 CONTRACT 审批返回")
    contract_file_name: Optional[str] = Field(None, description="合同附件原始文件名，仅 CONTRACT 审批返回")
    contract_file_size: Optional[int] = Field(None, description="合同附件大小，仅 CONTRACT 审批返回")
    contract_file_mime_type: Optional[str] = Field(None, description="合同附件 MIME 类型，仅 CONTRACT 审批返回")
    invoice_file_path: Optional[str] = Field(None, description="发票文件路径，仅 INVOICE 审批返回")
    invoice_number: Optional[str] = Field(None, description="发票号码，仅 INVOICE 审批返回")
    issued_time: Optional[str] = Field(None, description="发票开具时间，仅 INVOICE 审批返回")
