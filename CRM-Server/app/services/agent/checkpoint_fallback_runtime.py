...
class AgentCheckpointFallbackRuntime:
    ...
    def _start_fallback(self, _state: AgentRuntimeState) -> AgentRuntimeState:
        return {
    ...
    def _route_after_application_action(self, state: AgentRuntimeState) -> str:
        ...
        if action in {"execute_confirmed_task", "run_new_flow"}:
            return "checkpoint_write_blocked"
        if action == "no_pending_confirmation":
            return "no_pending_confirmation"
        return "finish_fallback"

    def _run_no_pending_confirmation(self, _state: AgentRuntimeState) -> AgentRuntimeState:
    ...
    def _checkpoint_write_blocked(self, state: AgentRuntimeState) -> AgentRuntimeState:
        assistant_content = (
            "当前 Agent checkpoint 存储不可用。为避免绕过审计、幂等和恢复机制，本轮业务写入已暂停；"  # noqa: RUF001
            "请稍后重试。"
        )
        return {
