"""Contact action-planning subgraph for the CRM Agent."""
...
from app.services.agent.state import (
    ContactPlanningGraphInput,
    ContactPlanningGraphResult,
    ContactPlanningGraphState,
    ContactPlanningRuntimeContext,
)
from app.services.agent.types import JSONDict, JSONValue, coerce_json_dict


CONTACT_CHECKPOINT_NS = "crm_agent_contact"

ContactCustomerRoute = Literal[
    "missing_customer_name",
    "single_customer",
    "multiple_customers",
    "customer_not_found",
]
ContactBusinessRoute = Literal[
    "collect_fields",
    "confirm_create",
]


def build_contact_thread_id(*, team_id: int, user_id: int, session_id: int) -> str:
    return f"crm_agent_contact:{team_id}:{user_id}:{session_id}"


def build_contact_graph_config(
    *,
    team_id: int,
    user_id: int,
    session_id: int,
        ...
        return {
            "configurable": {
                "thread_id": build_contact_thread_id(
                    team_id=team_id,
                    user_id=user_id,
            ...
            "user_id": user_id,
            "session_id": session_id,
            "runtime": "crm_agent_contact",
            "runtime_namespace": CONTACT_CHECKPOINT_NS,
        },
    }


class ContactPlanningGraphService:
    """Plans contact responses and HITL actions through explicit graph branches."""
    ...
    def _build_graph(self, checkpointer: object | None):
        graph = StateGraph(
            ContactPlanningGraphState,
            context_schema=ContactPlanningRuntimeContext,
        )
        ...
        graph.add_node("derive_contact_context", self._derive_contact_context)
        ...
        graph.add_conditional_edges(
            ...
            self._route_after_customer,
            {
                ...
                "single_customer": "derive_contact_context",
                "multiple_customers": "multiple_customers_response",
                "customer_not_found": "customer_not_found_response",
            },
        )
        graph.add_conditional_edges(
            "derive_contact_context",
            self._route_after_contact_context,
            {
    ...
    async def run(self, input_state: ContactPlanningGraphInput) -> ContactPlanningGraphResult:
        checkpoint_state = _checkpoint_state_from_input(input_state)
        context = ContactPlanningRuntimeContext(
        ...
        config = build_contact_graph_config(
        ...
        try:
            ...
            return with_checkpoint_unavailable_fallback_event(
                result,
                runtime="crm_agent_contact",
                graph=CONTACT_CHECKPOINT_NS,
            )

    def _derive_customer_route(
        self,
        state: ContactPlanningGraphState,
        runtime: Runtime[ContactPlanningRuntimeContext],
    ) -> ContactPlanningGraphState:
    ...
    def _route_after_customer(self, state: ContactPlanningGraphState) -> ContactCustomerRoute:
    ...
    def _missing_customer_name_response(self, state: ContactPlanningGraphState) -> ContactPlanningGraphState:
        return {"response": "我识别到这是创建联系人，但还缺少明确客户名称。请补充客户名称。", "action": {}}

    def _multiple_customers_response(self, state: ContactPlanningGraphState) -> ContactPlanningGraphState:
        candidates = state.get("customer_candidates") or []
        candidate_lines = [
        ...
        return {
            "response": "我找到了多个可能的客户，请告诉我要把联系人创建到哪一个客户：" + "；".join(candidate_lines),
            "action": {
    ...
    def _customer_not_found_response(self, state: ContactPlanningGraphState) -> ContactPlanningGraphState:
        return {
            "response": business_rules.customer_not_found_response(state.get("customer_name") or ""),
            "action": {},
        }

    def _derive_contact_context(
        self,
        state: ContactPlanningGraphState,
        runtime: Runtime[ContactPlanningRuntimeContext],
    ) -> ContactPlanningGraphState:
        missing_fields = business_rules.missing_contact_fields(state.get("contact") or {})
        return {
            "missing_fields": missing_fields,
            "contact_route": "collect_fields" if missing_fields else "confirm_create",
        }

    def _route_after_contact_context(self, state: ContactPlanningGraphState) -> ContactBusinessRoute:
        route = state.get("contact_route")
        if route in {"collect_fields", "confirm_create"}:
            return route
        return "collect_fields"

    def _collect_fields_response(self, state: ContactPlanningGraphState) -> ContactPlanningGraphState:
        customer = state.get("selected_customer") or {}
        missing_fields = state.get("missing_fields") or []
        return {
            "response": (
                f"我识别到要为「{customer.get('account_name')}」创建联系人，"
                f"还需要补充：{business_rules.format_contact_missing_fields(missing_fields)}。"
            ),
            "action": {
    ...
    def _confirm_create_response(self, state: ContactPlanningGraphState) -> ContactPlanningGraphState:
        customer = state.get("selected_customer") or {}
        contact = state.get("contact") or {}
        return {
            "response": (
...
def _checkpoint_state_from_input(input_state: ContactPlanningGraphInput) -> ContactPlanningGraphState:
    parsed = coerce_json_dict(input_state.get("parsed"))
    contact = coerce_json_dict(parsed.get("contact"))
...
contact_planning_graph_service = ContactPlanningGraphService(checkpointer=agent_checkpoint_saver)
