...
class WorkSummaryNarrativeService:
    ...
    async def summarize_with_metadata(
        ...
        *,
        ...
        if not grounded.answer and not grounded.highlights and not grounded.customer_summaries:
            return WorkSummaryNarrativeEnvelope(
                result=fallback,
                summary_source="deterministic_work_summary_fallback",
                model=config.model_name,
    ...
    def fallback_summary(self, *, question: str, work_facts: dict[str, Any]) -> WorkSummaryNarrativeResult:
        ...
        citations = self._citations_for_ids([str(item.get("fact_id")) for item in items], work_facts)
        if not items:
        ...
        highlights = [
            WorkSummaryNarrativeItem(
                ...
                title=str(item.get("title") or _fact_type_label(item)),
                summary=_fallback_fact_summary(item),
                fact_ids=[str(item.get("fact_id"))],
            )
            for item in items[:8]
            if item.get("fact_id")
        ]
        customer_summaries = self._customer_summary_items(items)
        lines = [f"### 工作总结\n围绕“{question or '当前工作'}”，系统查询到 {work_facts.get('available_total', len(items))} 条可确认事实。"]
        for item in highlights[:5]:
            lines.append(f"- **{item.title}**：{item.summary}")
        if work_facts.get("truncated"):
            lines.append("- 当前结果被截断，需要继续翻页后再生成完整总结。")
        return WorkSummaryNarrativeResult(
            answer="\n".join(lines),
    ...
    def _ground_result(
        ...
        valid_fact_ids = {str(item.get("fact_id")) for item in _fact_items(work_facts) if item.get("fact_id")}
        highlights = self._ground_items(result.highlights, valid_fact_ids)
        customer_summaries = self._ground_items(result.customer_summaries, valid_fact_ids)
        referenced_ids = _unique_texts([fact_id for item in [*highlights, *customer_summaries] for fact_id in item.fact_ids])
        citations = self._citations_for_ids(referenced_ids, work_facts)
    ...
    @staticmethod
    def _ground_items(
        items: list[WorkSummaryNarrativeItem],
        valid_fact_ids: set[str],
    ) -> list[WorkSummaryNarrativeItem]:
        grounded: list[WorkSummaryNarrativeItem] = []
        for item in items:
            fact_ids = _unique_texts([fact_id for fact_id in item.fact_ids if fact_id in valid_fact_ids])
            if not fact_ids:
                continue
            grounded.append(item.model_copy(update={"fact_ids": fact_ids}))
        return grounded
    ...
    def _customer_summary_items(items: list[dict[str, Any]]) -> list[WorkSummaryNarrativeItem]:
        ...
        for customer_name, customer_items in list(grouped.items())[:8]:
            fact_ids = [str(item.get("fact_id")) for item in customer_items[:5] if item.get("fact_id")]
            summaries.append(
                WorkSummaryNarrativeItem(
                    category="business_progress",
                    title=customer_name,
                    summary=f"{customer_name} 有 {len(customer_items)} 条工作事实，包括 {', '.join(_fact_type_label(item) for item in customer_items[:3])}。",
                    fact_ids=fact_ids,
                )
            )
        return summaries
    ...
    @staticmethod
    def _citations_for_ids(fact_ids: list[str], work_facts: dict[str, Any]) -> list[dict[str, object]]:
        facts_by_id = {str(item.get("fact_id")): item for item in _fact_items(work_facts) if item.get("fact_id")}
        citations: list[dict[str, object]] = []
        for fact_id in _unique_texts(fact_ids):
...
def _fact_type_label(item: dict[str, Any]) -> str:
    ...
    return labels.get(str(item.get("fact_type") or ""), "工作事实")


def _fallback_fact_summary(item: dict[str, Any]) -> str:
    customer = item.get("customer") if isinstance(item.get("customer"), dict) else {}
    customer_name = str(customer.get("name") or customer.get("account_name") or "未关联客户")
    occurred_at = str(item.get("occurred_at") or "")
    return f"{customer_name}：{_fact_type_label(item)}，时间 {occurred_at or '未记录'}。"
...
work_summary_narrative_service = WorkSummaryNarrativeService()
