"""customer activity next follow time source

Revision ID: 051_customer_activity_next_follow_time_source
Revises: 050_fix_migrated_meeting_activity_content
...
revision: str = "051_customer_activity_next_follow_time_source"
down_revision: str | None = "050_fix_migrated_meeting_activity_content"
...
def _create_customer_activities_table() -> None:
    op.create_table(
...
def _ensure_customer_activity_indexes() -> None:
...
def _backfill_from_customer_follow_ups() -> None:
    if not _table_exists("crm_customer_follow_ups"):
        return
    conn = op.get_bind()
    conn.execute(text("""
...
def _repair_deal_journey_events() -> None:
    if not _table_exists("crm_customer_follow_ups") or not _table_exists("crm_customer_deal_journey_events"):
        return
...
def upgrade() -> None:
    if not _table_exists("crm_customer_activities"):
        _create_customer_activities_table()
    _ensure_customer_activity_indexes()
    if not _column_exists("crm_customer_activities", "next_follow_time_source"):
        op.add_column(
            "crm_customer_activities",
            sa.Column("next_follow_time_source", sa.String(length=30), nullable=True, comment="下次跟进时间来源"),
        )
    _backfill_from_customer_follow_ups()
    _repair_deal_journey_events()
    op.get_bind().execute(text("""
        UPDATE crm_customer_activities
        SET next_follow_time_source = 'MIGRATED'
        WHERE next_follow_time IS NOT NULL
          AND next_follow_time_source IS NULL
    """))


def downgrade() -> None:
    if _table_exists("crm_customer_activities") and _column_exists("crm_customer_activities", "next_follow_time_source"):
        op.drop_column("crm_customer_activities", "next_follow_time_source")
