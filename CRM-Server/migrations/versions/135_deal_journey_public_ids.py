"""add public ids for deal journeys

Revision ID: 135_deal_journey_public_ids
Revises: 134_lead_customer_product_intent
...
revision: str = "135_deal_journey_public_ids"
down_revision: str | None = "134_lead_customer_product_intent"
...
def upgrade() -> None:
    table_name = "crm_customer_deal_journeys"
    index_name = "uq_crm_customer_deal_journeys_public_id"
    conn = op.get_bind()

    if not _column_exists(table_name, "public_id"):
        op.add_column(
            table_name,
            sa.Column("public_id", sa.String(length=64), nullable=True, comment="对外业务旅程ID"),
        )
    ...
    for row in rows:
        conn.execute(
            ...
            {"public_id": f"djy_{uuid4().hex}", "id": row["id"]},
        )
