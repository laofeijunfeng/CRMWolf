#!/usr/bin/env python3
"""Remove completed Agent task-list history after an approved recovery.

This destructive command is offline-only. It takes a MySQL WRITE lock across
all CRM tables, then removes only terminal Agent async-operation projections
and their event history. It does not delete business records, post-commit jobs,
or confirmation cases/deliveries.
"""
...
from app.services.agent.async_operation_history_cleanup import (
    AGENT_ASYNC_OPERATION_HISTORY_CLEANUP_SCHEMA_VERSION,
...
def _report_payload(
    *, status: str, result: AgentAsyncOperationHistoryCleanupResult | None, blockers: list[str]
) -> dict[str, object]:
...
def _read_completed_result(path: Path) -> AgentAsyncOperationHistoryCleanupResult:
    payload = json.loads(path.read_text(encoding="utf-8"))
    if (
        payload.get("schema_version") != AGENT_ASYNC_OPERATION_HISTORY_CLEANUP_SCHEMA_VERSION
    ...
    if not isinstance(raw_result, dict):
        raise AgentAsyncOperationHistoryCleanupError("agent_async_operation_history_cleanup:completed_evidence_invalid")
    try:
        return AgentAsyncOperationHistoryCleanupResult(
            schema_version=str(raw_result["schema_version"]),
            deleted_operation_count=_nonnegative_int(raw_result["deleted_operation_count"]),
            deleted_event_count=_nonnegative_int(raw_result["deleted_event_count"]),
        ...
        raise AgentAsyncOperationHistoryCleanupError(
            "agent_async_operation_history_cleanup:completed_evidence_invalid"
        ) from error
...
def _verify_completed_report(engine: Engine, path: Path) -> None:
    ...
    with engine.connect() as connection:
        AgentAsyncOperationHistoryCleanupService(connection).verify_committed_post_state(result)
...
def _journal_exists(engine: Engine) -> bool:
    with engine.connect() as connection:
        return AgentAsyncOperationHistoryCleanupService(connection).has_completed_journal()
...
def _mysql_lock_spec(connection: Connection) -> str:
    ...
    if not table_names:
        raise AgentAsyncOperationHistoryCleanupError("agent_async_operation_history_cleanup:mysql_tables_missing")
...
def _locked_cleanup_connection(engine: Engine) -> Iterator[Connection]:
    """Prevent any application writer from changing target rows during cleanup."""
    with engine.connect() as connection:
...
def _remove_stale_stage_if_uncommitted(engine: Engine, staged_path: Path) -> bool:
    ...
    except (OSError, AgentAsyncOperationHistoryCleanupError):
        return False
    return True
...
def _execute_locked(engine: Engine, *, output_path: Path) -> int:
    ...
    try:
        with _locked_cleanup_connection(engine) as connection:
            service = AgentAsyncOperationHistoryCleanupService(connection)
            service.run(
            ...
            staged_created = True
    except AgentAsyncOperationHistoryCleanupError as error:
        if staged_created:
            return 1
        ...
        with suppress(OSError):
            _write_payload(
                output_path,
                _report_payload(
                    status="FAILED", result=None, blockers=["agent_async_operation_history_cleanup:execution_failed"]
                ),
            )
        return 1
...
def execute_agent_async_operation_history_cleanup(engine: Engine, *, output_path: Path) -> int:
    """Serialize cleanup evidence and publish only after the committed state verifies."""
    _ensure_evidence_directory(output_path.parent)
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--offline-confirmed",
        action="store_true",
        help="Confirm that every application and background writer is stopped.",
    )
    parser.add_argument(
        "--accept-terminal-agent-task-history-loss",
        action="store_true",
        help="Acknowledge deletion of terminal Agent task-list projections and event history.",
    )
    args = parser.parse_args(argv)
    required_flags = ("execute", "offline_confirmed", "accept_terminal_agent_task_history_loss")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    return execute_agent_async_operation_history_cleanup(engine, output_path=args.output)
