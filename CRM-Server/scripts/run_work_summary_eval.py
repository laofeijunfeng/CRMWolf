"""Run deterministic work summary golden checks."""
...
from app.services.work_summary_golden_suite import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run deterministic work summary golden checks.")
    ...
    parser.add_argument(
        "--suite-name",
        default="work_summary_golden",
        help="Evaluation suite name",
    )
    args = parser.parse_args()

    cases_path = Path(args.cases_path) if args.cases_path else DEFAULT_WORK_SUMMARY_GOLDEN_CASES_PATH
    started_at = business_now()
    started_perf = time.perf_counter()
    summary = run_work_summary_golden_suite(cases_path)
